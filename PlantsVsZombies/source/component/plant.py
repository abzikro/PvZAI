...
class Bullet(pg.sprite.Sprite):
    def __init__(self, x, start_y, dest_y, name, damage, ice):
        ...
        self.rect.y = start_y
        self.dest_y = dest_y
        self.y_vel = 4 * c.SPEED_TIME if (dest_y > start_y) else -4 * c.SPEED_TIME
        self.x_vel = 4 * c.SPEED_TIME
        self.damage = damage
        self.ice = ice
        self.state = c.FLY
        self.current_time = 0
    ...
    def update(self, game_info):
        ...
        if self.state == c.FLY:
            ...
            if(self.current_time - self.explode_timer) > 500 / c.SPEED_TIME:
                self.kill()
...
class Plant(pg.sprite.Sprite):
    def __init__(self, x, y, name, health, bullet_group, scale=1):
        ...
        self.animate_interval = 100 / c.SPEED_TIME
        self.hit_timer = 0
    ...
    def animation(self):
        ...
        self.image = self.frames[self.frame_index]
        if(self.current_time - self.hit_timer) >= 200 / c.SPEED_TIME:
...
class Sun(Plant):
    def __init__(self, x, y, dest_x, dest_y, is_big=True):
        if is_big:
            ...
            scale = 0.6
            self.sun_value = 12
        Plant.__init__(self, dest_x, dest_y, c.SUN, 0, None, scale)
        self.move_speed = 1
        self.dest_x = dest_x
        self.dest_y = dest_y
        self.die_timer = 0
...
class SunFlower(Plant):
    def __init__(self, x, y, sun_group):
        ...
        self.map = Map(c.GRID_X_LEN, c.GRID_Y_LEN)
    
    def idling(self):
        if self.sun_timer == 0:
            self.sun_timer = self.current_time - (c.FLOWER_SUN_INTERVAL - 6000 / c.SPEED_TIME)
        elif (self.current_time - self.sun_timer) > c.FLOWER_SUN_INTERVAL:
            map_x, map_y = self.map.getRandomMapIndex()
            x, y = self.map.getMapGridPos(map_x, map_y)
            self.sun_group.add(Sun(self.rect.centerx, self.rect.bottom, x,y))
...
class PeaShooter(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) > 2000 / c.SPEED_TIME:
...
class CherryBomb(Plant):
    ...
    def animation(self):
        if self.start_boom:
            ...
            elif(self.current_time - self.bomb_timer) > 500 / c.SPEED_TIME:
                self.health = 0
        else:
            if (self.current_time - self.animate_timer) > 100 / c.SPEED_TIME:
                self.frame_index += 1
                if self.frame_index >= self.frame_num:
                    self.setBoom()
                    return
...
class Chomper(Plant):
    def __init__(self, x, y):
        ...
        self.animate_interval = 250 / c.SPEED_TIME
        self.digest_timer = 0
        self.digest_interval = 15000 / c.SPEED_TIME
...
class PuffShroom(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) > 3000 / c.SPEED_TIME:
...
class PotatoMine(Plant):
    def __init__(self, x, y):
        ...
        self.animate_interval = 300 / c.SPEED_TIME
        self.is_init = True
        self.init_timer = 0
        self.bomb_timer = 0
    ...
    def idling(self):
        if self.is_init:
            ...
            elif (self.current_time - self.init_timer) > 15000 / c.SPEED_TIME:
                self.changeFrames(self.idle_frames)
                self.is_init = False
...
class Squash(Plant):
    ...
    def attacking(self):
        ...
        elif (self.current_time - self.aim_timer) > 1000 / c.SPEED_TIME:
            self.changeFrames(self.attack_frames)
...
class Spikeweed(Plant):
    def __init__(self, x, y):
        ...
        self.animate_interval = 200 / c.SPEED_TIME
        self.attack_timer = 0
    ...
    def setAttack(self, zombie_group):
        self.zombie_group = zombie_group
        self.animate_interval = 50 / c.SPEED_TIME
        self.state = c.ATTACK

    def attacking(self):
        if (self.current_time - self.attack_timer) > 2000 / c.SPEED_TIME:
...
class SunShroom(Plant):
    ...
    def idling(self):
        if not self.is_big:
            ...
            elif (self.current_time - self.change_timer) > 25000 / c.SPEED_TIME:
...
class IceShroom(Plant):
    ...
    def animation(self):
        if self.start_freeze:
            if(self.current_time - self.animate_timer) > 500 / c.SPEED_TIME:
                self.frame_index += 1
                if self.frame_index >= self.frame_num:
                    self.health = 0
                    return
...
class WallNutBowling(Plant):
    def __init__(self, x, y, map_y, level):
        ...
        self.animate_interval = 200 / c.SPEED_TIME
        self.move_timer = 0
        self.move_interval = 70 / c.SPEED_TIME
