...
plant_frozen_time_list = np.array([7500, 7500, 7500, 30000, 50000, 7500, 7500, 7500, 7500, 30000,
                          30000, 7500, 50000, 7500, 7500, 50000, 30000, 0, 0]) / c.SPEED_TIME
...
def getIndexByPlantName(name):
    for i in range(len(plant_name_list)):
        if plant_name_list[i] == name:
            return i
...
class Panel():
    def __init__(self, card_list, sun_value, select_cards=False):
        self.setupCards(card_list)
        self.selected_cards = []
        self.selected_num = 0
        if select_cards:
            self.select_cards_automatically()
        self.loadImages(sun_value)


    def select_cards_automatically(self):
        selected_card_indexes = [0,1,2,3,4,5,6,7]
        for card in self.card_list:
            if self.selected_num == CARD_LIST_NUM:
                return
            if card.canSelect() and card.name_index in selected_card_indexes:
                self.addCard(card)
