...
class Zombie(pg.sprite.Sprite):
    ...
    def walking(self):
        ...
        if (self.current_time - self.walk_timer) > (c.ZOMBIE_WALK_INTERVAL * self.getTimeRatio()):
            ...
            if self.is_hypno:
                self.rect.x += self.speed * c.SPEED_TIME
            else:
                self.rect.x -= self.speed * c.SPEED_TIME
    ...
    def attacking(self):
        if self.health <= 0:
            ...
            self.helmet = False
        if (self.current_time - self.attack_timer) > (c.ATTACK_INTERVAL * self.getTimeRatio() / c.SPEED_TIME):
    ...
    def animation(self):
        ...
        if (self.current_time - self.animate_timer) > (self.animate_interval * self.getTimeRatio() / c.SPEED_TIME):
            self.frame_index += 1
