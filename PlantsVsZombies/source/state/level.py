...
class Level(tool.State):
    def __init__(self, mode):
        ...
        self.mode = mode
    def startup(self, current_time, persist, randomized=False):
        self.game_info = persist
        self.persist = self.game_info
        self.game_info[c.CURRENT_TIME] = current_time
        self.map_y_len = c.GRID_Y_LEN
        self.map = map.Map(c.GRID_X_LEN, self.map_y_len)
        self.loadMap(randomized)
        self.setupBackground()
        self.initState()
        self.menubar = None
        self.num_of_dead_plants = 0
        # self.num_of_dead_zombies = 0

    def loadMap(self, randomized=False):
        map_file = 'level_' + str(self.game_info[c.LEVEL_NUM]) + '.json'
        file_path = os.path.join("PlantsVsZombies", 'source', 'data', 'map', map_file)
        f = open(file_path)
        self.map_data = json.load(f)
        for i in range(len(self.map_data['zombie_list'])):
            self.map_data['zombie_list'][i]['time'] /= c.SPEED_TIME
            if randomized:
                random_time = np.random.normal()
                if random_time > 1:
                    random_time = 1
                if random_time < -1:
                    random_time = -1
                random_time *= (10000 / c.SPEED_TIME)
                self.map_data['zombie_list'][i]['time'] += random_time
                if np.random.random() < 0.5:
                    self.map_data['zombie_list'][i]['map_y'] = np.random.randint(5)
        f.close()
    ...
    def initState(self):
        ...
        if self.bar_type == c.CHOOSEBAR_STATIC:
            if self.mode != c.MANUAL:
                self.initChoose(True)
                self.initPlay(self.panel.getSelectedCards())
            else:
                self.initChoose(False)
        else:
    ...
    def initChoose(self, select_cards):
        self.state = c.CHOOSE
        self.panel = menubar.Panel(menubar.all_card_list, self.map_data[c.INIT_SUN_NAME], select_cards)
    ...
    def get_game_state(self):
        ...
        # Process zombies
        for i, zombie_group in enumerate(self.zombie_groups):
            for zombie in zombie_group:
                x, y = self.map.getMapIndex(zombie.rect.centerx, zombie.rect.bottom)
                key = f"{x},{y}"
                if key not in state['zombies']:
                    state['zombies'][key] = []
                state['zombies'][key].append({
                    'type': zombie.name,
        ...
        for i, plant_group in enumerate(self.plant_groups):
            for plant in plant_group:
                x, y = self.map.getMapIndex(plant.rect.centerx, plant.rect.bottom)
                key = f"{x},{y}"
                state['plants'][key] = {
                    'type': plant.name,
                    'health': plant.health,
        ...
        # Process sun
        for sun in self.sun_group:
            x, y = self.map.getMapIndex(sun.rect.centerx, sun.rect.bottom)
            state['sun'].append((x,y))

        cars_pos = [car.map_y for car in self.cars]
        for i in range(self.map_y_len):
            state['cars'][i] = i in cars_pos

        return state

    def print_state_info(self):
        print("Current State Information:")
        print(f"Sun Value: {self.menubar.sun_value}")

        print("\nPlants:")
        for i, group in enumerate(self.plant_groups):
            for plant in group:
                print(f"Row {i + 1}: {plant.name} at ({plant.rect.x}, {plant.rect.y}), Health: {plant.health}")

        print("\nZombies:")
        for i, group in enumerate(self.zombie_groups):
            for zombie in group:
                print(f"Row {i + 1}: {zombie.name} at ({zombie.rect.x}, {zombie.rect.y}), Health: {zombie.health}")

        print("\nSuns:")
        for sun in self.sun_group:
            print(f"Sun at ({sun.rect.x}, {sun.rect.y}), Value: {sun.sun_value}")
    ...
    def get_plants_data(self):
        # Return simplified data about all plants
        pass

    def get_zombies_data(self):
        # Return simplified data about all zombies
        pass


    def place_plant(self, plant_type, position, cost):
        # Place a plant and return success/failure
        self.plant_name = plant_type
        x, y = self.map.getMapGridPos(position[0], position[1])
        map_x, map_y = self.map.getMapIndex(x,y)
        if self.plant_name == c.SUNFLOWER:
            new_plant = plant.SunFlower(x, y, self.sun_group)
        elif self.plant_name == c.PEASHOOTER:
        ...
        self.plant_groups[map_y].add(new_plant)
        if self.bar_type == c.CHOOSEBAR_STATIC:
            self.menubar.decreaseSunValue(cost)
            self.menubar.setCardFrozenTime(self.plant_name)
        else:
            self.menubar.deleateCard(self.select_plant)
    ...
    def collect_sun(self, position):
        # Collect sun at the given position
        pos = self.map.getMapGridPos(position[0], position[1])
        for sun in self.sun_group:
            if sun.checkCollision(pos[0], pos[1]):
                self.menubar.increaseSunValue(sun.sun_value)
                return True
        return False

    def get_plant_card(self, plant_ind):
        return self.menubar.checkCardClick(self.map.getCardsPos(plant_ind))

    def is_cell_empty(self, x, y):
        return self.map.isMovable(x, y)

    def get_card_cost(self, plant_type):
        pos = self.map.getCardsPos(menubar.getIndexByPlantName(plant_type))
        result = self.menubar.checkCardClick(pos)
        return result[1].sun_cost

    def get_card_cost_without_check(self, plant_type):
        pos = self.map.getCardsPos(menubar.getIndexByPlantName(plant_type))
        result = self.menubar.getCard(pos)
        return result[1].sun_cost

    def get_sun_value(self):
        return self.menubar.sun_value
